...
d <- read_xlsx('Judo variability analysis - Index adjusted 2.xlsx',
  ...
  filter(sex != 'Male') %>% # Deselect males
...
# Save
# ggsave(file = "x_correlations.png",
#        width = 6,
#        height = 5,
#        dpi = 600)
...
# round(cor_boot(dsub$throws_per_match, dsub$ground_techniques_per_match)$conf.int,2)
...
# Refit with redundant parameter (weight) removed
fit <- brm(medalist_y ~ age_s + sport_class + throws_per_match_s + ground_techniques_per_match_s + penalties_per_match_s,
          family = bernoulli(link = "logit"),
          data = dsub,
          prior = set_prior('horseshoe(df = 1, scale_global = 1)', class = "b"),
          cores = 8,
...
# Visualise parameter estimates and save
posterior <- as.matrix(fit)
color_scheme_set('darkgray')
mcmc_areas(posterior,
           pars = c('b_age_s',
           ...
           prob = 2/3,
           point_est = 'median')+
  geom_vline(xintercept = 0, colour = 'darkorange3', size = 0.5)+
  theme_bw()+
  theme(panel.grid = element_blank())+
  facet_grid(~'Parameter estimates')+
  labs(x = 'Effect (logit)',
       y = 'Parameter')
...
#write.csv(df, file = 'cv_final_model_women.csv')
df <- read.csv('cv_final_model_women.csv')
...
# Save
ggsave(file = "roc_women.png",
       width = 5,
       height = 4,
       dpi = 600)

# Calibration plot
val.prob.ci.2(df$pred, df$y_var)
